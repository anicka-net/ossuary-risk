...
21 out-of-scope incidents are included in the dataset to validate detection boundaries. All score below 60 as expected, except ua-parser-js (bonus detection):
...
The v6.4 dataset extension (May 2026, n=170 → n=177) added seven
incidents from the May 2026 sweep without any methodology changes.
Four new in-scope cases all scored as TPs: `jest-canvas-mock` and
`timeago.js` (T1 governance decay, Shai-Hulud dormant-package
campaign), `fsnotify/fsnotify` (T_risk maintainer dispute), and
`node-ipc` second incident (T3 inactive-maintainer compromise). Three
out-of-scope cases joined as expected FNs: `@tanstack/router` (T5
CI/CD exploit), `pytorch-lightning` (T4 build-pipeline breach), and
`laravel-lang/lang` (reclassified T3→T4 after scoring revealed healthy
governance). Scope B recall rose from 75.0% to 77.8% (28/36) and
F1 from 0.828 to 0.848 through pure dataset composition — no model
changes.
...
*Report generated from validation run on May 30, 2026*
